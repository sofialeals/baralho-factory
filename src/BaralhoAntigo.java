...
public class BaralhoAntigo
{
   private List<CartaAntiga> deck; // ArrayList usado como uma pilha de objetos
...
//      String naipe[] = { "Hearts", "Diamonds", "Clubs", "Spades" };  
      ...
      deck = new ArrayList<CartaAntiga>(); // cria List de objetos Card
      randomNumbers = new Random();  // cria gerador de n�mero aleat�rio

      // preenche baralho com objetos Card
      for ( int count = 0; count < NUMBER_OF_CARDS; count++ )   {
         deck.add(new CartaAntiga( faces[ count % 13 ], naipe[ count / 13 ], 0, naipe[ count / 13 ] == "\u2665" || naipe[ count / 13 ] == "\u2666" ? Cor.VERMELHO: Cor.PRETO ));
      }
   ...
   public void shuffle()
   {
      ...
      for ( int first = 0; first < deck.size(); first++ )
      {
         ...
         CartaAntiga temp = deck.remove( second );
         deck.add(0, temp);
      } // for final
   } // fim do m�todo shuffle

   // retira uma carta, do topo do monte
   public CartaAntiga dealCard()
   {
